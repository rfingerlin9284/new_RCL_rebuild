...
class CoinbaseConnector:
    ...
    def market_order(self, product_id: str, side: str, size: float, price: float | None = None) -> Dict[str, Any]:
        """Lightweight market order interface for router compatibility.
        Sandbox: simulate immediate fill.
        Live: submit a MARKET order via API.
        Charter min notional enforcement is skipped in sandbox to allow tiny test trades.
        """
        start = time.time()
        try:
            if self.environment == "live":
                order_data = {
                    "type": CoinbaseOrderType.MARKET.value,
                    "side": side,
                    "product_id": product_id,
                    "size": str(size)
                }
                resp = self._make_request("POST", "/orders", order_data)
                if not resp.get("success"):
                    return {
                        "success": False,
                        "error": resp.get("error", "LIVE market order failed"),
                        "broker": "Coinbase",
                        "environment": self.environment,
            ...
            # Sandbox simulation
            import random
            latency = random.uniform(25, 120)
            time.sleep(latency / 1000)
            order_id = f"CB_MKT_{product_id}_{int(time.time()*1000)}"
