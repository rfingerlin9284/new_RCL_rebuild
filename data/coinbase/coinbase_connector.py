...
class CoinbaseConnector:
    ...
    def _get_headers(self, method: str, endpoint: str, body: str = "") -> Dict[str, str]:
        """
        Generate authenticated headers for Coinbase API requests
    ...
    # ========== TRADING ENGINE API METHODS ==========
    # Methods expected by coinbase_trading_engine.py
    
    def connect(self) -> bool:
        """
        Establish connection to Coinbase API (stateless REST, always succeeds)
        Returns True if credentials are configured
        """
        if not self.api_key or self.api_key == "your_sandbox_api_key":
            self.logger.warning("Coinbase API credentials not configured")
            return False
        
        try:
            # Test connection with a lightweight API call
            response = self._make_request("GET", "/products/BTC-USD")
            self.logger.info(f"✅ Connected to Coinbase {self.environment} API")
            return response.get("id") == "BTC-USD"
        except Exception as e:
    ...
    def disconnect(self):
        """
        Disconnect from Coinbase API (stateless REST, no-op)
        """
        self.logger.info("Coinbase connector disconnect (stateless REST)")
    
    def get_historical_candles(self, symbol: str, granularity: int = 300, count: int = 60) -> List[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            count: Number of candles to fetch (max 300)
        ...
        try:
            ...
            if isinstance(response, list):
                # Convert Coinbase format to standard OHLCV dict
                candles = []
                for candle in response[:count]:
                    if len(candle) >= 6:
                        candles.append({
                            "time": candle[0],
                            "open": candle[3],
                ...
                self.logger.error(f"Unexpected candles response format: {type(response)}")
                return []
    ...
    def get_open_positions(self) -> List[Dict[str, Any]]:
        """
        Get all open orders/positions from Coinbase
        
        Returns:
            List of position dicts with keys: id, product_id, side, size, price, type
        """
        try:
            # Coinbase Advanced Trade API: /orders (filter by status=open)
            endpoint = "/orders"
            params = {"status": "open"}
            ...
            if isinstance(response, dict) and "orders" in response:
                ...
                self.logger.warning(f"Unexpected positions response: {response}")
                return []
    ...
    def close_position(self, symbol: str) -> Dict[str, Any]:
        """
        Close all open positions for a product by placing reverse market orders
        ...
        try:
            ...
            symbol_positions = [p for p in positions if p["product_id"] == symbol]
            ...
            self.logger.error(f"❌ Failed to close positions for {symbol}: {e}")
            return {"success": False, "error": str(e)}
    
    def place_order(self, product_id: str, side: str, size: float, price: float = None,
                   stop_loss: float = None, take_profit: float = None, 
                   order_type: str = "market") -> Dict[str, Any]:
        """
        Place order (wrapper that uses existing place_oco_order for OCO support)
        
        Args:
            product_id: Symbol (e.g., 'BTC-USD')
            side: 'buy' or 'sell'
            size: Order size
            price: Entry price (for OCO, uses current market if None)
            stop_loss: SL price (required for OCO)
            take_profit: TP price (required for OCO)
            order_type: 'market' or 'limit'
        
        Returns:
            Order result dict
        """
        # If SL and TP provided, use OCO order
        if stop_loss and take_profit:
            entry_price = price if price else 0  # Will fetch market price in place_oco_order
            ...
            # Simple market/limit order (not OCO)
            self.logger.warning("Simple order without OCO protection - not recommended")
            return {
                "success": False,
                "error": "Orders without SL/TP not supported (Charter violation)"
            }
